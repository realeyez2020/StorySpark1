...
class AiClient {
  ...
  Future<Map<String, dynamic>> generateCompleteStory({
    ...
    try {
      final response = await _client.post(
        Uri.parse('$baseUrl/story/complete'),
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to generate complete story: ${response.statusCode}');
      }
    } catch (e) {
      throw Exception('Error generating complete story: $e');
    }
  }
  ...
  Future<Uint8List> generatePageImage({
    ...
    try {
      final response = await _client.post(
        ...
        body: jsonEncode({
          'page': page,
          'lines': [scene], // Backend expects 'lines' as array
          'styleLabel': userStyle, // Backend expects 'styleLabel'
          'genreLabel': genreLabel, // Pass actual genre
          'art_bible': artBible,
...
class MockAiClient extends AiClient {
  ...
  Future<Map<String, dynamic>> generateCompleteStory({
    ...
    final characterName = storyBible['characters']?[0]?['name'] ?? 'Sam';
    
    return {
      'title': 'The Adventure of $characterName',
      'character': {'name': characterName, 'description': 'A curious child'},
      'pages': List.generate(numPages, (i) => {
        'page': i + 1,
        'lines': ['$characterName began the adventure.', 'Something exciting happened.'],
        'scene_hint': '$characterName in a magical scene'
      })
    };
  }
